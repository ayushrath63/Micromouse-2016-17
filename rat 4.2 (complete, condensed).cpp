...
int main() {
    ...
    while(1)
    {
        ...
        if (IR_array[1] < IRF_THRESHOLD || IR_array[2] < IRF_THRESHOLD) //if it sees wall
        {
            ...
            if (IR_array[1] > IRF_THRESHOLD2 || IR_array[2] > IRF_THRESHOLD2)//if the distance is bigger than 2nd threshold contine
                forward(270,tspeed);
            calibrateFront();
            wait(1);
            myled = 1; wait(0.1); myled = 0;
        ...
        //forward relying on both wall, rely on the wall that has the least discrepancy from the standard
        else
        {
            float LeftIRDifference = IR_array[1]-IR_array_past[1]; //left discrepancy from the standard
            float RightIRDifference = IR_array[1]-IR_array_past[1]; //right discrepancy from the standard
            if (abs(RightIRDifference) < 3)//if the discrepancy is small for right, rely on right wall (??)
            {
                MRF = speed + PID(IR_array[0],IR_array_past[0],0.2,0.008,0.002);
                MLF = speed - PID(IR_array[0],IR_array_past[0],0.2,0.008,0.002); //HERE
            }
            else if (abs(LeftIRDifference)< 3)//if the
            {
                MRF = speed + PID(IR_array[3],IR_array_past[3],0.2,0.008,0.002);
                MLF = speed - PID(IR_array[3],IR_array_past[3],0.2,0.008,0.002); //HERE
            }
            else
            {
                MRF = speed;
